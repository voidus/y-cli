...
class ChatApp:
    ...
    def split_content(self, content: str) -> tuple[str, Optional[str]]:
        ...
        if first_tag_index < len(content):
            plain_content = content[:first_tag_index].strip()
            tool_content = content[first_tag_index:].strip()
    ...
    async def process_response(self, response_stream) -> str:
        """Process streaming response and handle tool use"""
        assistant_response = self.stream_response(response_stream)
        
        if not self.contains_tool_use(assistant_response):
            ...
            plain_content, tool_content = self.split_content(assistant_response)
            assistant_message = {
                "role": "assistant",
            ...
            mcp_tool = self.extract_mcp_tool_use(assistant_response)
            if mcp_tool:
            ...
            final_response = self.stream_response(response)
            assistant_message = {
                "role": "assistant",
        ...
        async with AsyncExitStack() as exit_stack:
            try:
                ...
                while True:
                    ...
                    user_message = {"role": "user", "content": user_input, "timestamp": get_iso8601_timestamp(), "unix_timestamp": get_unix_timestamp()}
                    self.messages.append(user_message)
