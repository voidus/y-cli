...
@cli.command()
@click.option('--chat-id', '-c', help='ID of the chat to share (defaults to latest chat)')
def share(chat_id: Optional[str]):
    """Share a chat conversation by generating a shareable link.
    
    If no chat ID is provided, shares the most recent chat.
    """
    ...
    try:
        ...
        # Upload to S3
        os.system(f'aws s3 cp "{tmp_file}" s3://{config["s3_bucket"]}/ask/{chat_id}.html > /dev/null')
        
        # Invalidate CloudFront cache
        os.system(f'aws cloudfront create-invalidation --distribution-id {config["cloudfront_distribution_id"]} --paths "/ask/{chat_id}.html" > /dev/null')
        
        # Print the shareable URL
        click.echo(f'https://{config["s3_bucket"]}/ask/{chat_id}.html')
